...
for program in PROGRAMS:
    ...
    for version in VERSIONS:
        ...
        model_period_ns = SINGLE_MODEL_PERIOD_NS if version == "single" else PIPELINE_CONFIGURED_PERIOD_NS
        ...
        summary.append({
            **row,
            ...
            "configured_period_ns": model_period_ns,
            "configured_cpu_time_us": model_time_ns / 1000.0,
            "configured_speedup_vs_single": single_model_time_ns / model_time_ns,
...
with (RESULTS / "summary.csv").open("w", encoding="utf-8-sig", newline="") as handle:
    fields = ["program", "version", "cycles", "retired", "cpi", "stalls", "branches",
              ...
              "same_clock_speedup_vs_single", "configured_period_ns", "configured_cpu_time_us",
              "configured_speedup_vs_single", "prediction_gain_vs_off"]
    writer = csv.DictWriter(handle, fieldnames=fields, extrasaction="ignore")
    writer.writeheader()
    for row in summary:
        formatted = dict(row)
        for field in ("cpi", "same_clock_cpu_time_us", "same_clock_speedup_vs_single",
                      "configured_cpu_time_us", "configured_speedup_vs_single"):
...
lines = [
    ...
    "时间主视图采用配置频率：单周期基线 10 MHz（100 ns），流水线 50 MHz（20 ns）。流水线完整整机已在 20 ns 约束下完成布局布线，WNS=+6.527 ns；统一 10 MHz 的同频结果同时保留作对照。",
    "单周期核心同样通过了 50 MHz 时序检查，因此 10/50 MHz 是课程展示的配置频率，不是两种设计各自最高频率的比较。",
    "动态指令数包含实际完成的分支和存储，不包含空泡、暂停等待或错误路径指令。",
    "",
    "| 程序 | 版本 | 周期 C | 指令 N | CPI | 暂停 | 配置周期/ns | 配置 CPU 时间/us | 配置加速比 |",
    "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
]
for row in summary:
    lines.append(
        f"| {program_labels[row['program']]} | {labels[row['version']]} | {row['cycles']} | "
        f"{row['retired']} | {row['cpi']:.3f} | {row['stalls']} | {row['configured_period_ns']} | "
        f"{row['configured_cpu_time_us']:.3f} | {row['configured_speedup_vs_single']:.3f} |"
    )
...
lines += [
    ...
    "- `CPI = C / N`",
    "- `配置 CPU 时间 = C × 配置时钟周期`",
    "- `单周期基线配置周期 = 100 ns；流水线配置周期 = 20 ns`",
    "- `配置加速比 = 单周期基线 CPU 时间 / 流水线 CPU 时间`",
    "- `预测收益 = 关闭预测流水线时间 / 开启预测流水线时间`",
    "",
    "## 同频 10 MHz 对照",
    "",
    "同频对照用于分离CPI和暂停影响。完整数字保留在 `summary.csv` 的 `same_clock_*` 字段中；在同为50 MHz时，当前无前递流水线并不快于单周期基线。",
    "",
    "## 自动一致性检查",
    "",
    "- 9 组仿真均通过各自的功能结果检查。",
    ...
    "- 配置时间由已验证周期数乘以明确标注的100 ns/20 ns配置周期得到。",
    "- 流水线完整整机50 MHz布局布线时序已通过；开发板JTAG当前未连接，上板功能仍待复查。",
]
