...
module hazard_tb;
 ...
 initial begin
  ...
  imem[11]=32'h0000006f; #12 resetn=1; wait(dmem[4]==10); #30;
  if(dmem[0]==10&&dmem[1]==0&&dmem[2]==15&&dmem[3]==0&&dmem[4]==10&&writes==3)
    $display("HAZARD_PASS writes=%0d",writes);
  else $fatal(1,"HAZARD_FAIL m=%0d,%0d,%0d,%0d,%0d writes=%0d",dmem[0],dmem[1],dmem[2],dmem[3],dmem[4],writes);
  done=1;$finish;
 end
 initial begin #5000;if(!done)$fatal(1,"HAZARD_TIMEOUT");$finish;end
endmodule
