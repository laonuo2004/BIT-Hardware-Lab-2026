# ============================================================
# sort_uart.asm — UART 交互式多数据集排序（黄奕晨）
#
#   1 已排序 1 2 3 4 5
#   2 逆序   5 4 3 2 1
#   3 重复   3 1 3 2 1
...
# RAM：a[0..4] @ 0x00，last_case @ 0x14，custom 备份 @ 0x20
# ============================================================
    .text
    .globl main
...
wait_digit:
    lw   t0, 4(a0)
    andi t0, t0, 2
    beq  t0, x0, wait_digit
    lw   t0, 8(a0)
    addi t1, x0, 0x0D
    beq  t0, t1, wait_digit
    addi t1, x0, 0x0A
    beq  t0, t1, wait_digit
    addi t1, x0, 0x30
    blt  t0, t1, bad_digit
...
send_digit:
    lw   s10, 0x14(x0)
    addi t1, x0, 5
    beq  s10, t1, send_letter_s
    addi s10, s10, 0x30
    jal  x0, send_digit_wait
send_letter_s:
    addi s10, x0, 0x53
send_digit_wait:
    lw   t0, 4(a0)
